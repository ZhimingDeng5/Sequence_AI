...
from Sequence.sequence_model import COORDS, SequenceState, SequenceGameRule
from Sequence.sequence_utils import TRADSEQ, EMPTY, HOTBSEQ, JOKER, MULTSEQ, RED, BLU
    ...
    def bfs(self, beginsta, agent_id):
            ...
            for next1pos in spositions:
                for next2pos in positions:
                    if next2pos not in spositions:
                        if abs(next1pos[1] - next2pos[1]) + abs(next1pos[0] - next2pos[0]) <= 3:
                            return True
        ...
        while len(lis) > 0:
            ...
            for action in valid_actions:
                nstate, reward, cur_agent_index = sim.execute(copy.deepcopy(statenow), action, self.id)
...
class SimulateGame(SequenceGameRule):
     # use the created simulateing game
